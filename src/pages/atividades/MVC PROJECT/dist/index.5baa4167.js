...
const clickEsquerdo = (event)=>{
    ...
    let confirm = window.confirm("O usu\xe1rio selecionado ser\xe1 carregado para edi\xe7\xe3o!");
    if (confirm) {
...
const clickDireito = (event)=>{
    ...
    if (event.button == 2) {
        currentId = event.target.closest("tr").id.split("")[4];
        alert(`O ${data[currentId].getNome().toUpperCase()} ser\xe1 deletado!`);
        let confirm = window.confirm("Certeza que quer deletar esse usu\xe1rio?");
        if (confirm) {
            deletUser(currentId);
            (0, _viewControllerJs.viewController).update(data, new (0, _usuarioModelJs.Usuario)("", null, "", ""));
        }
...
const resultView = {
    ...
    update: (params)=>{
        ...
        for(let i = 0; i < params.length; i++){
            ...
            newLine.innerHTML = `
        <th scope="row">#${i + 1}</th>            
        <td class="text-success"><strong ><b>${params[i].getNome()}</b></strong></td>            
        <td><strong>${params[i].getIdade()}</strong></td>            
        <td><b class="text-info"><i>${params[i].getLogin()}</b></i></td>            
        <td><input class="form-control" type="password" value="${params[i].getSenha()}" disabled style="border: none !important;
